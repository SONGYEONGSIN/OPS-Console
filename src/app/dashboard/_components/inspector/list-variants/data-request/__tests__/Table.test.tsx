...
describe("DataRequestTable", () => {
  it("대학명/서비스명/운영/개발 렌더", () => {
    render(
      <DataRequestTable rows={[row()]} selectedId={null} onSelect={() => {}} />,
    );
  ...
  it("빈 목록 안내", () => {
    render(
      <DataRequestTable rows={[]} selectedId={null} onSelect={() => {}} />,
    );
  ...
  it("작성시작 컬럼(연도 제외) 표시", () => {
    render(
      <DataRequestTable
        rows={[row({ writeStartAt: "2026-05-11T00:00:00+09:00" })]}
        selectedId={null}
        onSelect={() => {}}
      />,
    );
  ...
  it("작성시작이 지난 행은 클릭해도 onSelect 미호출(음영)", () => {
    ...
    render(
      <DataRequestTable
        rows={[
          row({ id: "p1", universityName: "과거대학교", writeStartAt: past }),
        ]}
        selectedId={null}
        onSelect={onSelect}
      />,
  ...
  it("발송이력 있으면 일자+시간(KST) 표시", () => {
    ...
    render(
      <DataRequestTable
        rows={[
          row({
            writeStartAt: "2026-05-11T00:00:00+09:00",
            dataRequestLastSentAt: "2026-05-23T05:30:00Z",
          }),
        ]}
        selectedId={null}
        onSelect={() => {}}
  ...
  it("작성시작이 안 지난 행은 클릭하면 onSelect 호출", () => {
    ...
    render(
      <DataRequestTable
        rows={[
          row({ id: "f1", universityName: "미래대학교", writeStartAt: future }),
        ]}
        selectedId={null}
        onSelect={onSelect}
      />,
...
/**
 * 예약완료는 발송 전 상태(backup 예약완료와 같은 뜻)라 idle 톤이어야 한다 — 붉은 배지로
 * 남아 있으면 backup과 어긋난다(#948 리뷰). 발송완료는 정상 종료라 done 톤.
 */
describe("DataRequestTable — 자료요청 상태 배지 톤", () => {
  it("예약완료는 idle 톤이다", () => {
    render(
      <DataRequestTable
        rows={[
    ...
    expect(screen.getByText("예약완료").className).toContain(BADGE_TONE.idle);
  });
  it("발송완료는 done 톤이다", () => {
    render(
      <DataRequestTable
    ...
    expect(screen.getByText("발송완료").className).toContain(BADGE_TONE.done);
  });
});
