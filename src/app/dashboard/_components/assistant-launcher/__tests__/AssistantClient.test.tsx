...
/**
 * 기본은 Claude(회사 PC)라, 즉답 경로를 확인하려면 토글을 꺼야 한다.
 * 렌더와 함께 쓰는 헬퍼로 묶어 각 테스트에서 반복하지 않는다.
 */
...
describe("AssistantClient — Claude 모드", () => {
  ...
  it("기본이 Claude다 — 지식망을 직접 읽은 답이 기본이어야 한다", () => {
    render(<AssistantClient />);
    const toggle = screen.getByRole("button", { name: /Claude/ });
  ...
  it("도는 동안 무엇을 하고 있는지 보여준다 — 30초를 '답변 중'만 보면 멈춘 줄 안다", async () => {
    ...
    await waitFor(() => expect(screen.getByText(/회사 PC로 보냈습니다/)).toBeInTheDocument(), {
      timeout: 5000,
    });
    await waitFor(() => expect(screen.getByText(/지식망 문서를 읽는 중/)).toBeInTheDocument(), {
      timeout: 8000,
    });
  }, 15000);
