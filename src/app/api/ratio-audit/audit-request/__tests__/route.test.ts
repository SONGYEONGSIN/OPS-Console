...
// supabase 쿼리 빌더 mock — 체인 메서드는 자기 자신 반환, await/maybeSingle은 state.result.
function builder() {
...
function get(secret?: string) {
  return new Request("http://localhost/api/ratio-audit/audit-request", {
    method: "GET",
...
describe("/api/ratio-audit/audit-request", () => {
  beforeEach(() => {
    vi.clearAllMocks();
