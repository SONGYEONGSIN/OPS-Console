...
/**
 * 개발 탭 원서제어 '수동 분석' 폴러 endpoint — `Authorization: Bearer ${CRON_SECRET}` 인증.
 * 회사 PC 폴러(scripts/dev-control/poll-local.ps1)가 호출한다.
 *   GET  → 가장 오래된 pending 1건을 원자적 claim(→running). 없으면 { request: null }.
 *   POST → 완료 보고 { id, ok, message } → done/failed.
 * 쓰기는 service_role(admin client)로만. closing/scrape-request와 동형.
 */
