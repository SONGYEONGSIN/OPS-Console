...
describe("SPECIAL_MAP — 운영 발견 alias", () => {
  ...
  // 입금 거래내용 '명지인문대학원'(명지대학교 산하)도 명지대로 매핑.
  // 2026-08-19 확인 요청 2건(120,000 / 55,000)이 이 표기 차이로 보류됐다.
  it("명지인문대학원 → 명지대", () => {
    expect(normalizeName("명지인문대학원")).toBe("명지대");
  });

  it("명지인문대학원 입금이 명지대학교 미수와 강매칭된다", () => {
    expect(isNameMatchStrong("명지대학교", "명지인문대학원")).toBe(true);
  });
