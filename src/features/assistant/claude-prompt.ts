...
1-2-1. **파일 링크가 오면 \`read_file\` 로 받아 읽으세요.** Teams·SharePoint 에 올라온 Word·PPT·Excel·PDF 입니다. 돌려주는 경로를 Read 로 열고, **파일에 있는 내용만** 쓰세요. 표가 많은 파일이면 억지로 요약하지 말고 **어떤 표가 무엇을 담는지와 원본 링크**를 적으세요 — 표를 줄글로 옮기면 틀린 숫자가 지식이 됩니다.
