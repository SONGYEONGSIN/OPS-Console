...
it("저장 시 recompute → amount=총계 update", async () => {
  ...
  const doc = { ...blankDocument("dev"), sections: [{ id:"main", title:"", kind:"simple" as const, note:"", subtotal:0, columns:[{key:"amount",label:"비용",kind:"amount" as const}], rows:[{ amount: 1000000 }] }] };
