  ...
  return {
    ...
    members: team.members.map((m) => {
      ...
      if (m.source.kind === "poller") {
        ...
        //
        // 이름 뒤에 '폴러' 를 붙인다 — 잡과 폴러가 **같은 파이프라인의 두 각도**라
        // 라벨이 거의 같았다(`서비스 마감 스크래핑` vs `마감 스크래핑`). 둘 다
        // 있어야 하는 이유는 고장 지점이 달라서다: PC 가 죽으면 폴러가 멈추고,
        // 스크래핑이 깨지면 잡에 실패가 남는다. 원본 라벨은 안 건드린다 —
        // 시스템 상태 화면은 폴러만 모아 두어 접미사가 군더더기다.
        return {
          ...base,
          detail: `${poller.label} 폴러`,
          planned: false,
          pollerId,
          driver: "요청 대기",
        };
