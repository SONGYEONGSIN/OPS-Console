...
import { createAiTip, updateAiTip, deleteAiTip } from "../actions";
...
describe("createAiTip", () => {
  it("viewer → 차단", async () => {
    mockGetCurrentOperator.mockResolvedValue(viewerMe);
    const r = await createAiTip(validInput);
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.error).toMatch(/권한/);
    expect(mockInsert).not.toHaveBeenCalled();
  });

  it("zod 실패 (reuse_prompt 누락) → 거부", async () => {
    mockGetCurrentOperator.mockResolvedValue(memberMe);
    const { reuse_prompt: _omit, ...broken } = validInput;
    const r = await createAiTip(broken);
    expect(r.ok).toBe(false);
    expect(mockInsert).not.toHaveBeenCalled();
  });
  ...
  it("member → 통과 (author_email 자동 채움)", async () => {
    ...
    mockInsert.mockResolvedValue({
      data: { id: "x", ...validInput, author_email: "member@x.com" },
      error: null,
    });
    const r = await createAiTip(validInput);
...
describe("updateAiTip", () => {
  it("본인 글 update → 통과", async () => {
    ...
    const r = await updateAiTip("x", { title: "수정" });
    expect(r.ok).toBe(true);
  });
  ...
  it("target 없음 → not found", async () => {
    ...
    const r = await updateAiTip("none", { title: "수정" });
    expect(r.ok).toBe(false);
  });
});

describe("deleteAiTip", () => {
