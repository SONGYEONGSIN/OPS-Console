...
export const FEATURE_INTROS: FeatureIntro[] = [
  ...
  },
  {
    menu: "AI & 자동화 > 업무 지식망",
    title: "업무 지식망",
    desc: "우리가 아는 것을 한곳에 모아 둡니다. 탭이 넷이에요. **문서**는 쌓인 지식을 분류별로 봅니다 — 개념·플레이북·규칙·결정·오류사례·엔티티·프로젝트. \"경위서 어떻게 보내지?\" 같은 게 플레이북에 있고, 대학별 사정은 엔티티에 있습니다. **초안 만들기**는 재료를 던지면 문서를 지어 줍니다 — Teams·SharePoint 링크를 붙여넣거나, 내 PC 파일을 올리거나, 메일 본문처럼 파일이 없으면 그냥 붙여넣으면 됩니다. **검토 대기**는 그렇게 만들어진 초안입니다. 사람이 읽고 맞으면 [지식망 옮기기]를 눌러야 정식 문서가 돼요 — 잘못된 내용이 그대로 쌓이면 어시스턴트가 그걸 근거로 답하게 되니까요. **빈틈**은 어시스턴트가 답하다 \"이건 문서에 없다\"고 남긴 자리입니다. 무엇을 더 적어야 할지 알려주는 목록이에요. 여기 쌓인 문서를 상황실 어시스턴트(명보)가 직접 읽고 답합니다.",
  },
  {
