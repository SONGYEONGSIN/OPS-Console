...
export const AUTOMATION_JOBS: AutomationJob[] = [
  ...
  },
  {
    ...
    label: "학년도 배정 요청",
    description:
      "새 학년도의 배정 제안을 만들도록 판정 요청을 적재합니다. 그 학년도 배치가 이미 있으면 건너뜁니다.\n판정은 회사 PC 폴러(Agent SDK)가 5분 내 가져가 수행하고, 결과는 업무배정 > 제안 탭에 배치로 쌓입니다 — 적용 전까지 원장은 바뀌지 않습니다.\n연간 배정은 3월에 한 번 하는 일이라 자동 스케줄이 없습니다. 업무배정 > 제안 탭의 [연간 제안 만들기]나 이 버튼을 눌러야 요청이 적재됩니다.",
    /*
     * **수동 전용이다**(사용자 2026-09-20). 업무 흐름이 *2월 종료 → 3월 연간 배정 →
     * 이후 모니터링* 이고, 2027학년도는 이미 배정이 끝났다. 매일 돌면 관리자가
     * 부르지도 않은 제안이 쌓이고, 그건 *"신규 서비스 배정 요청이 없으면 굳이
     * 제안해 줄 필요가 없다"* 와 정면으로 어긋난다.
     *
     * `manual` 로 두면 미실행 감지(`digest.ts` 의 `STALE_AFTER_HOURS.manual = null`)
     * 에서 빠지는데, **그게 목적이다** — 안 누르면 안 도는 것이 정의다. 대신 3월
     * 갱신 상기는 배경 알림이 아니라 버튼을 누른 그 화면에 뜬다.
     */
  ...
  },
  {
    ...
    label: "배정 미배정 감지",
    description:
      "배정 원장에서 담당자가 없는 (대학 × 업무종류)가 있는지 평일마다 확인해 건수로 알립니다.\n이름은 있는데 메일 주소가 안 이어진 칸('연결 안 됨')도 함께 셉니다 — 고칠 것은 주소이지 배정이 아닙니다.\n판정 요청은 만들지 않습니다: 누구에게 줄지는 업무배정 > 신규배정에서 사람이 정합니다.",
