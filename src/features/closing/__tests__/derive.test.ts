...
describe("imminentClosings — 작성마감 D-3 이내", () => {
  it("write_end_at이 오늘(D-0)인 건을 포함한다", () => {
    const rows = [row({ write_end_at: "2026-06-16T18:00:00+09:00" })];
    expect(imminentClosings(rows, TODAY)).toHaveLength(1);
  });

  it("write_end_at이 D-3(오늘+3)인 건을 포함한다", () => {
    const rows = [row({ write_end_at: "2026-06-19T09:00:00+09:00" })];
    expect(imminentClosings(rows, TODAY)).toHaveLength(1);
  });

  it("write_end_at이 D-4(오늘+4)인 건은 제외한다", () => {
    const rows = [row({ write_end_at: "2026-06-20T09:00:00+09:00" })];
    expect(imminentClosings(rows, TODAY)).toHaveLength(0);
  });

  it("write_end_at이 어제(이미 마감)인 건은 제외한다", () => {
    const rows = [row({ write_end_at: "2026-06-15T23:59:00+09:00" })];
    expect(imminentClosings(rows, TODAY)).toHaveLength(0);
  });

  it("write_end_at 오름차순으로 정렬한다", () => {
    const rows = [
      row({ id: "c", write_end_at: "2026-06-18T09:00:00+09:00" }),
      row({ id: "a", write_end_at: "2026-06-16T09:00:00+09:00" }),
      row({ id: "b", write_end_at: "2026-06-17T09:00:00+09:00" }),
    ];
