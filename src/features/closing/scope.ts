import type { ServicePhase } from "./phase";
/**
 * 같은 목록(`closing_services`)을 네 메뉴가 생애주기로 나눠 본다.
 *
 * - **개발 · 테스트**(`upcoming`) — 아직 접수 시작 전. 열기 전에 테스트한다.
 * - **배포 · 운영**(`running`) — 접수 중. 지금 돌보는 대상이다.
 * - **서비스마감**(`closed`) — 결제까지 끝난 것.
 * - **전형료정산**(`settlement`) — 마감된 것을 정산 관점으로 본다.
 *
 ...
 */
export type ClosingScope = "upcoming" | "running" | "closed" | "settlement";
...
export function resolveScopeFilter(
  ...
  return {
    // 칩이 무엇이든 범위는 메뉴가 정한다. 정산은 마감된 것을 본다.
    phase: scope === "settlement" ? "closed" : scope,
