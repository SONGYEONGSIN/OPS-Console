...
**T3-b — 긁는 쪽 (미착수)**: Moa 의 어느 메뉴, 어느 표에서 정산 금액을 읽는지 아직 정해지지 않았다. 코드에 알려진 Moa 경로는 `/User/Login` 과 `/Foundation/ServiceSearch` 둘뿐이고, 서비스검색 엑셀 14컬럼에 금액은 없다. 화면을 확인한 뒤 시작한다.
