...
**그래서 첫 단계는 스크래퍼가 아니라 디스커버리다.** `scripts/moa-settlement/discover.py` 가 아무것도 가정하지 않고 화면의 링크·입력칸·버튼 `onclick`·표 머리글을 그대로 보고한다. 회사 PC 에서 `run-discover.ps1` 을 한 번 돌린 출력이 곧 위 둘의 답이며, 그것을 받아야 스크래퍼를 쓴다. 자세한 건 `scripts/moa-settlement/README.md`.
