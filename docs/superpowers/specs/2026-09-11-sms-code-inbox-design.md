...
| F3 | **코드 2개 연속 수신**(버튼 두 번, Moa 재발송) | 로그에 `우편함에 2건 — 최신을 씁니다` | 최신을 꺼낸다. Moa가 재발급 시 이전 코드를 무효화하므로 최신이 정답 |
| F4 | **시계 어긋남**(폰 시각 오차) | 드러나지 않는다 — **영향이 없다** | 폰 시각을 받지 않는다. `received_at`은 서버 `now()` |
| F5 | **Supabase 장애 / DB 오류** | `[WARN] 우편함 준비 실패 — make 웹훅으로 넘어갑니다: …` | **make 1순위 → 2순위.** 이번 설계의 폴백이 값을 하는 유일한 칸 |
