...
| 순서 | 건 | 판단 |
|---|---|---|
| **완료** | **문서-코드 불일치** — `closing_services` 적재 전략을 "덮어씀"으로 적은 3곳(`CLAUDE.md`, `20260607c` 마이그 주석, `target-url.ts:6`). 실제는 `upsert ignoreDuplicates` 누적 | 2026-08-22 정정 완료. **이 설계의 FK 판단을 잘못 세우게 만든 원인**이었다. `target-url.ts` 는 폴백의 *근거* 가 틀렸던 것이라(폴백 자체는 맞음) 근거만 바꿨다 |
...
| **안 만든다** | **R6** `sending` 고착 복구 자동화 | 갇히는 경로가 둘인데 구분이 안 된다 — (a) 발송 전에 죽음(되돌리는 게 맞음) (b) 발송 성공 + update 실패(되돌리면 대학에 두 번 간다). 자동 복구는 이중 발송 위험이라 **넣지 않는다.** 복구 SQL은 dispatch 라우트 주석에 남겼다(디버깅하는 사람이 보는 자리). 실측 0건 |
