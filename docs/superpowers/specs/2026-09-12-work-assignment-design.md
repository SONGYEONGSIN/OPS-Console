...
| 03. 대학원 | 대학원 | 없음 | 있음 |
| 04. PIMS | PIMS | `FULL` / `환충` | **없음** |
| 06. 성적산출 | 성적산출 | 없음 | 있음 |
| 07. 상담앱 | 상담앱 | 없음 | 있음 |

> **PIMS 시트에는 운영자 칸이 셋인데 배정은 둘이다**(2026-09-15 라이브 실측, 대학 81곳). 헤더는 `No · 대분류 · 지역 · 대학명 · 서비스구분 · 담당자 변경 · **운영자 FULL**(6) · 접수운영자(7) · **운영자 환/충**(8) · 前 운영자(9) · 비고`.
>
> **`접수운영자`는 배정이 아니라 사본이다** — 채워진 70건이 `02. 배정리스트`의 **수시** 담당자와 **70/70(100%) 일치**했다(대조군 `운영자 FULL`은 같은 칸과 43%만 겹친다). 원장에 넣으면 같은 배정이 두 벌이 되므로 읽지 않는다. `前 운영자`(81/81 채움)는 과거값이라 마찬가지다. 사용자 확인 완료(2026-09-15).
>
> 채움 실측은 **FULL만 71 · 환충만 10 · 둘 다 0 · 둘 다 빔 0**. 둘 다 찬 행이 0건인 것은 **오늘 데이터의 성질이지 불변식이 아니다.** 그래서 `parsePims`가 `operator = full || hwan`으로 접던 것을 걷고 **`subtypes`로 두 칸을 따로 내보낸다**(2026-09-15). 접힌 값을 배정의 단위로 쓰면 FULL 배정이 조용히 사라지는데, **대조는 양쪽이 같은 시트에서 나오므로 그것을 구조적으로 못 잡는다.** 하위유형 라벨은 `FULL`·`환충` 고정이다 — 시트 헤더를 따라 `환/충`으로 적으면 자연키가 갈려 기존 행이 고아가 된다.
...
|---|---|---|
| 원서접수 | `closing_services` 중 **`category` 에 `대학원` 이 없는 것** | 창 안 573건 / 268곳 |
| 대학원 | `closing_services` 중 **`category` 에 `대학원` 이 든 것** — `대학원 후기` 260 · `대학원` 89 · `대학원 전기` 42 · 법학전문대학원 5 | 창 안 396건 / 52곳 |
...
> **rev 3 (2026-09-17, PR5 구현 중 실측으로 정정) — 원천은 `closing_services` 하나다.**
>
> rev 2 는 원서접수 건수를 `services` 에서 센다고 적었는데 **그 표는 2026-02-28 에서 멈춰 있다**: 2511행 전부 `source='google_sheet_import'` 이고 `write_start_at` 최대값이 `2026-02-28`, 즉 지금 학년도 창(2026-03-01~2027-02-28)에 **0건**이다. 그대로 쓰면 전원의 건수·밀도·주월연이 통째로 0 이 되고, 화면에서 그 0 은 '일이 없다' 로 읽힌다. 살아 있는 미러는 `closing_services` 이고 창 안 **969건/286곳** — rev 2 가 적은 **286곳과 정확히 같다.** 원서접수와 대학원은 같은 표에 있고 `category` 로 갈린다.
>
> **주·월·연 진행도 같은 이유로 `closing_services.write_start_at ~ write_end_at` 이다.** rev 2 가 적은 `services` 는 여기서도 0건이다.
>
> **건수의 단위는 (대학 × 업무종류)다.** 대학 단위로 세면 갈린 44곳(§3.1)에서 남의 PIMS 건수가 원서접수 담당자에게 붙는다 — 286곳 중 15% 라 밀도가 통째로 거짓이 된다.
>
> **0 을 두 가지로 쓰지 않는다.** 원천에 그 조합이 없어 못 센 칸을 줄마다 따로 적는다(`WorkloadRow.uncounted` → 표의 `건수 (N칸 못 셈)`). 못 세는 자리가 셋이다 — 원천이 아예 없는 업무종류(성적산출 44곳·상담앱 25곳), 원장에는 있는데 원천에 이름이 없는 대학(원서접수 293곳 중 32곳·대학원 49곳 중 16곳), 그리고 **아직 시즌이 안 온 발표**(PIMS 81곳 중 창 안에 잡히는 곳 32 — `last_announce_at` 이 서비스당 '가장 최근' 한 칸이라 12월 발표 전에는 창에 안 든다). 이 셋이 0 으로 뭉개지면 그 사람은 한가해 보인다.
...
> **rev 4 (2026-09-18, PR6 구현) — 이동의 단위는 (대학 × 업무종류)다.**
>
> 위 문단은 응답 한 줄에 **하위유형·역할**도 적으라고 했는데, 둘 다 이동의 단위로 두면 안 된다.
>
> - **하위유형**: 원장은 하위유형마다 한 줄이라 한 줄만 옮기면 수시는 김, 정시는 이가 된다 — **그게 바로 C4 가 막으려던 분할**이고 C6("대학은 통째로 옮긴다")과도 어긋난다. 그래서 `applyMoves` 는 그 (대학, 업무종류)의 모든 하위유형을 함께 옮기고, 모델이 하위유형마다 한 줄씩 보내면 `parse-response` 가 합친다(**서로 다른 담당자를 말하면 거부**한다).
> - **역할**: 개발 칸은 `operators` 밖이라(PR4b) `assignable` 판정 자체가 불가능해 배정 대상이 될 수 없다. 이동은 언제나 운영 칸이고, 갈림 판정(G4·단건)도 운영 칸만 본다 — 개발 이름이 다르다고 갈린 것으로 세면 멀쩡한 대학이 전부 손댈 수 없게 된다.
>
> **G2 는 `size === 1` 을 보지 않는다.** 하위유형이 갈린 칸을 경합으로 떨구면 보고에 '원장과 다르다' 가 남아 사람이 원장을 들여다보는데, 실제로는 갈린 건이라 손댈 것이 없다. 갈림은 G4 가 제 이름으로 떨군다.
>
> **게이트 평가 순서는 G1→G2→G3→G4→G7→G5→G6 이다.** 번호순이 아닌 이유는 G5(상한)·G6(Σdev)이 **누적 상태**를 보기 때문이다 — 어차피 떨어질 줄이 상한을 깎아먹으면 뒤 줄이 엉뚱한 이유로 탈락한다.
...
> **rev 5 (2026-09-18, PR7a 구현) — 적재 잠금은 `ratio_audit_requests` 보다 좁다.**
>
> 경쟁률 점검의 잠금은 **kind 무관 전역**이다(pending/running 이 하나라도 있으면 막는다). 그건 그쪽 두 점검이 **같은 Moa 로그인**을 타서, 동시에 돌면 SMS 두 통이 한 폰에 겹쳐 서로의 코드를 가져가기 때문이다. 판정은 Moa 를 안 만진다.
>
> 그 모양을 그대로 베끼면 **평일 미배정 감지가 조용히 아무것도 적재하지 않는다** — 3월의 annual 판정이 `running` 인 동안 그날의 단건 요청이 전부 `skipped` 로 떨어지고, 잡 이력에는 '성공(건너뜀)' 만 남는다. 그래서 잠금을 **같은 요청**으로 좁혔다: `academic_year` + `kind`, 단건이면 `university_name` + `work_kind` 까지(`propose-requests/enqueue.ts`). 막으려던 것은 **같은 판정이 두 벌 도는 것**이고, 그건 이 범위로 충분하다.
>
> **`STALE_RUNNING_MS`(30분)는 폴러의 `JUDGE_TIMEOUT_MS`(10분)보다 커야 한다.** 작으면 서버가 죽은 줄 알고 큐를 열어 같은 판정이 두 벌 돈다 — 두 값은 묶여 있고 한쪽만 고치면 안 된다.
...
> **rev 6 (2026-09-18, PR7b 구현) — 두 잡이 학년도를 다른 데서 가져온다.**
>
> rollover 는 `currentAcademicYear()`(시계)다 — 새 학년도가 시작된 것을 알아야 하는 잡이라 시트 헤더에 박힌 `BAEJUNG_CURRENT_YEAR` 로는 3월이 와도 안 움직인다. 대신 **원장이 비어 있으면 적재하지 않는다**: 빈 원장으로 판정하면 제안 0건짜리 배치가 생기고 그 순간 '그 학년도 배치가 있다' 가 되어 **진짜 배정은 영영 제안되지 않는다.** 총괄장 이관이 먼저다.
>
> 미배정 감지는 `BAEJUNG_CURRENT_YEAR` 다 — **화면에 떠 있는 그 원장**을 훑는 것이고 대학배정 화면이 그 상수를 본다. 시계로 도출하면 3월에 한 해를 건너뛰어, 화면엔 `미배정` 배지가 떠 있는데 잡은 빈 원장을 보고 '미배정 없음' 을 보고한다. 두 값이 같은 오늘은 테스트가 갈림을 못 잡아서 **원문으로 고정했다**(import 대조).
>
> **'원장에 없는 서비스'(새 대학)는 미배정 감지에서 뺐다.** PR5 실측에서 서비스 원천과 원장의 이름이 안 맞는 키가 48곳(원서접수 32 · 대학원 16)이고 그 숫자는 매일 같다 — 평일 보고에 올리면 늘 켜진 경고등이 되어 진짜 변화를 덮는다. 대학명 갈림은 `대조` 버튼이 드러내는 자리가 이미 있고, 별칭을 추측해 잇지 않는 것이 F1 이다.
>
> **'연결 안 됨'(이름은 있고 주소가 없는 칸)은 요청하지 않고 건수만 보고한다.** 그 칸에는 사람이 적어 둔 담당자가 있어 고칠 것은 주소이지 배정이 아니다(F2) — 판정으로 보내면 에이전트가 이미 담당자가 있는 칸에 남을 앉힌다.
...
```
r0: 대학명 | 대분류 | 2027 운영자(5) | 2027 개발자(5) | 대학원(2) | PIMS(2) | 성적산출(2) | 상담앱(2)
r1:       |        | 재외 수시 정시 편입 외국인 | (동일) | 운영 개발 | FULL 환충 | 운영 개발 | 운영 개발
```

> **rev 3 (PR8 구현, 2026-09-21) — 위 표가 rev 2 에서 세 군데 틀렸다.** 원장 1,858행을 실측해 고쳤다.
>
> | 틀린 것 | 실측 | 고친 것 |
> |---|---|---|
> | PIMS 를 `운영 개발` 로 적음 | `FULL 71` · `환충 10` · **개발 0건** | **`FULL 환충` 2칸, 둘 다 운영자.** §3.1 이 옳았고 이 표가 틀렸다 |
> | 원서접수 블록에 `백업자` 포함(6칸) | 원장에 백업자 행이 **없다** | **5칸.** 백업자는 배정이 아니라 공백 때 대신 볼 사람이고, `parse.ts:118` 이 원장에서 명시적으로 가른다. 원장에 없는 것을 열로 만들면 영원히 빈 칸이다 |
> | 두 학년도 고정(2027·2026) | 원장에 **2027 만** 있다 | **원장에 있는 학년도만**(최신순 최대 2개). 2026 열 20칸을 빈 채로 내면 *"2026 배정이 전부 비었다"* 는 거짓말이 된다 — 안 실은 것과 비어 있는 것이 구분되지 않는다 |
>
> 상한 2개는 유지한다(`EXPORT_MAX_YEARS`). 학년도가 늘면 열이 오른쪽으로 밀리는 것도 그대로다(결정 3).

`02. 배정리스트`의 2줄 헤더 모양을 그대로 재현한다 — 눈에 익은 형태가 곧 이관의 비용을 줄인다. 원서접수 외 4종을 같은 시트에 합치는 이유는 화면의 대학배정 표와 같은 모양이기 때문이다(대학 한 행에 전부 보인다).

**최대 두 학년도를 싣는다**(원장에 있는 것 중 최신순). `02` 시트가 오늘 2027/2026 두 해만 갖고 있는 것과 같고, 셋 이상은 DB에서 본다 — 시트는 산출물이라 원천의 전부를 실을 의무가 없다. **없는 해의 열은 만들지 않는다**(rev 3). 학년도가 하나 늘면 열이 오른쪽으로 밀린다(그게 결정 3이다).
...
> **rev 6 (2026-09-18, PR7b 구현) — `행별 적용` 은 두지 않는다.**
>
> 위 문단은 rev 4(이동의 단위가 (대학 × 업무종류))보다 먼저 쓰였다. 제안 **행**은 원장 한 줄이라 하위유형마다 하나인데, 한 줄만 적용하면 수시는 김·정시는 이가 된다 — **그게 정확히 G4 가 막으려던 분할**이고, 게이트가 거부한 것을 화면 버튼이 허용하는 꼴이다. 열려면 단위가 (대학 × 업무종류)여야 하고 그건 별도 설계다. 지금은 `전체 적용` / `반려` 둘뿐이다.
>
> **적용은 한 번 더 묻고 반려는 안 묻는다.** 반려는 원장을 안 건드려 되돌릴 것이 없고(R4), 적용은 원장을 바꾼다. 비대칭이 곧 '반려가 기본' 이라는 구조다.
>
> **경합 줄은 `pending` 으로 남긴다.** 배치는 `partial` 이 되고 그 줄들은 사람이 원장을 보고 다시 판단한다 — `rejected` 로 닫으면 왜 안 들어갔는지가 사라진다.
>
> **판정 성공의 Teams 보고는 창구(POST)가 보낸다.** 판정은 잡이 아니라서 성공에 `automation_runs` 줄이 안 생기고 일일 보고에도 안 잡힌다 — 이 메시지가 없으면 배치가 제안 탭 안에만 있고, 그 탭을 열어 볼 이유가 아무에게도 생기지 않는다. 연 1회 배정이 그렇게 조용히 안 된다.
...
- **파일**: `page.tsx`, `_row-mapper.ts`, `assignments/{View,EditForm}.tsx`, `registry.ts`, `actions.ts`, `ListPattern.tsx`, `ImportAssignments.tsx`(**이관 버튼 → 대조 버튼으로 교체**), tests 2
...
- **RED(이관 버튼 철거)**: **원장에 쓰는 경로가 화면에서 사라진다** — `importAssignments` 를 부르는 컴포넌트가 없다(호출처 0을 단언한다) / 남는 `reconcileAssignments` 는 **시트와 원장을 읽기만 한다**(admin 클라이언트를 쓰지 않는 것을 단언한다 — 세션 클라이언트에는 쓰기 정책이 없어 이게 곧 쓰기 불가의 증거다) / 비-admin 거부는 그대로
- **검증**: `npm test` · `npm run typecheck` · `npm run lint` · 화면에서 한 칸 변경 → 이력 → 되돌리기 1회 · **대조 버튼을 누른 뒤 `assignments`·`assignment_changes` 행수가 그대로**(읽기 전용 증거)
- **의존**: PR3

> **왜 쓰기를 걷는가**(2026-09-15 결정). PR4 부터 편집이 앱에서 일어나는데 이관은 자연키 `onConflict` upsert 라 **시트에 있는 모든 칸을 시트 값으로 되돌린다.** 클릭 한 번이 앱 편집을 전부 덮고, `assignment_changes` 는 append-only 라 그 덮어씀이 **정당한 변경으로 이력에 남으며 `updated_by` 는 버튼 누른 사람**이 된다 — 되돌릴 수도 사고로 구분할 수도 없다. 게다가 **대조가 이것을 못 잡는다**: 쓰기 다음에 돌면서 방금 덮어쓴 원장과 시트를 비교하니 0건으로 통과한다(§13 R1 — **대조는 양쪽이 같은 원천에서 나오면 눈이 먼다.** PR #1193 이 고친 PIMS 접힘과 같은 맹점이다).
>
> "복구용으로 남긴다"는 성립하지 않는다. PR4 뒤의 시트는 이미 낡아서 전량 재가져오기는 복구가 아니라 두 번째 사고다. 복구는 편집기 + 되돌리기이고 그쪽은 이력을 남긴다. **철거는 §12-4 라이브 이관이 통과한 뒤에 한다** — 통과 전에 걷으면 이관 수단이 사라진다.
>
> **철거가 데려가는 테스트 3건은 `updateAssignment` 가 이어받는다.** 이관 action 이 지우면 그 테스트도 지워지는데, 셋 다 **이관이 아니라 원장에 쓰는 행위**의 불변식이라 편집기에도 그대로 걸린다 — 옮겨 적지 않으면 같은 사고를 다시 만든 뒤에 알게 된다.
>
> 1. **자연키로 쓴다**(`onConflict: academic_year,university_name,work_kind,subtype,role`) — 편집이 `insert` 로 가면 같은 칸이 두 행이 되고, 그 다음부터 원장 조회가 어느 쪽을 줄지 모른다
> 2. **이력은 바뀐 칸만이고 단위는 이메일이다** — `prev === next` 면 남기지 않는다. 저장을 두 번 눌러 이력이 두 줄 늘면 되돌리기가 무엇을 되돌리는지 알 수 없다
> 3. **이메일이 없는 칸은 이력을 남길 수 없다** — `prev=next=null` 이 되어 `assignment_changes_actual_change_chk` 가 `23514` 로 적재를 통째로 죽인다. 이름만 바꾸는 편집은 원장만 고치고 이력 줄을 만들지 않는다
>
> 쓰기 실패를 삼키지 않는 것도 같이 간다 — supabase-js 는 던지지 않으므로 `error` 를 안 보면 '저장했습니다' 를 띄우고 끝난다.

> **편집기 쪽 결정 넷**(2026-09-17). PR4 를 읽기·철거(4a)와 편집·이력·되돌리기(4b)로 나눠 머지했고, 4b 에서 정한 것들이다.
>
> 1. **개발 칸은 자유 입력이고 이력에 남지 않는다**(사용자 결정). `operators` 가 운영부 표라 개발자는 구조적으로 못 들어가고, 이력의 단위는 이메일이다 — 이름만 바꾸는 편집은 원장의 `assignee_name` 만 고친다. 이름 이력을 남기려면 `assignment_changes` 에 칸을 더하는 마이그레이션이 필요해서 뺐다.
> 2. **담당자 교체·비우기만 한다.** 원장에 없는 칸(새 하위유형)을 만드는 것은 거부한다 — (업무종류 × 하위유형) 유효 조합 표가 없어서 열면 오타가 아무도 안 보는 칸을 만든다. 원장이 방금 총괄장에서 왔으므로 실제 배정에는 이미 칸이 있다.
> 3. **운영 칸의 이름 스냅샷은 명부에서 온다.** 폼이 보낸 이름을 쓰지 않는다 — 낡은 이름(개명·오타)이 들어오면 화면이 한 사람을 두 이름으로 부른다.
> 4. **이력은 그 페이지에 뜬 대학만 읽는다**(`listAssignmentChanges(year, names)`). 학년도 전체를 읽으면 편집이 쌓일수록 목록 한 장을 그리는 비용이 자란다. 상한을 넘기면 **던진다** — 조용히 자르면 오래된 이력이 사라진 것처럼 보이고, 그 자리에서 되돌리기를 누른 사람은 무엇을 되돌리는지 모른다.
>
> 5. **운영 칸 후보는 `status='active'` 뿐이다**(`assignmentCandidates`). 처음엔 "화면만 좁히면 서버 판정과 갈린다"며 안 좁혔는데, 그건 **서로 다른 세 질문을 한 목록에 뭉쳐서** 생긴 가짜 딜레마였다 — 누구를 **새로 배정**할 수 있나(정책·active), 이 이메일이 **누구인가**(사실·명부 전원), 이 주소로 **되돌릴 수 있나**(FK·명부 전원). 셋을 가르면 후보를 좁혀도 갈리지 않는다.
>
> `assignable` 로 좁히지는 않는다. 그 칸은 **자동 배정**의 대상 여부이고(§3.4 — 팀장·부장·이사·기획팀·테스트 계정이 false), 수동 편집기는 자동이 못 다루는 예외를 처리하는 자리다. 후보 목록을 active 로 두는 것은 레포 관례이기도 하다(`backupOperators`·`servicesOperators`·`aiWorkOperators`).
>
> 좁히면 **후보에서 빠진 사람이 들고 있는 칸**이 생기는데, 그 사람을 그 칸의 후보에 남긴다(`(지금 담당)`). 안 남기면 select 값이 어느 옵션과도 안 맞아 `비움` 으로 보이고 이름 힌트는 이메일이 없을 때만 떠서 **이름까지 사라진다** — 폼을 연 시점의 담당자를 스냅샷으로 기억하므로, 다른 사람으로 바꿨다가 되돌릴 수도 있다.
...
| 4 | 시트 → DB 이관 **1회** 실행 + 대조 0건 | 배정 화면 admin 버튼 — **1회용이고 PR4 에서 걷는다**(§13 R1) | 화면이 빈다 |
...
| # | 리스크 | 처리 |
|---|---|---|
| R1 | 이관 후 사람이 `02` 시트를 고쳐 DB와 갈린다 | **재가져오기를 만들지 않는다**(DB를 덮어쓰는 버튼은 원장을 파일에 종속시킨다). **PR4 가 편집기를 실으면 쓰기 버튼을 걷고 읽기 전용 `대조` 만 남긴다**(사용자 결정 2026-09-15) — 갈림을 만들지 않으면서 갈림을 탐지한다. 시트에 안내를 넣을지는 열린 질문 3 |
...
| **`02` 시트 직접 쓰기** | §7.1 A안 |
| **시트 → DB 재가져오기(쓰기)** | R1. **읽기 전용 대조는 범위 안이다** — 쓰지 않으므로 R1 을 만들지 않고, 오히려 R1 이 적은 위험(사람이 시트를 고쳐 갈린다)을 드러내는 유일한 장치다 |
...
>
> **PR5 에서 닫힌 것 하나**: 배분현황 권한(Q2 — admin only).
...
2. ~~**배분현황을 전원에게 열어도 되는가?**~~ **닫힘 (2026-09-17) — admin only.** 남의 업무량을 견주는 표라 열람/편집을 가르는 것과 같은 선에 둔다. 대학 수는 어차피 누구나 셀 수 있고(원장 select 가 `using (true)` 이며 `내 배정` 은 기본값일 뿐인 필터다), 새로 가려지는 것은 **건수·밀도** 뿐이다. 탭 목록에서 빼는 것으로 끝내지 않고 `?tab=workload` 로 들어와도 `univ` 로 떨어뜨린다 — 목록에서만 빼면 주소를 아는 사람에게는 열려 있는 것과 같다.
3. **이관 후 `02. 배정리스트` 시트를 어떻게 하는가?** 방치하면 누군가 고치고 DB와 갈린다(F6·R1). 선택지: (a) 시트 1행에 '원천은 OPS-Console' 안내, (b) 시트 보호, (c) 방치. 재가져오기는 만들지 않는 편이 맞다고 본다. **(부분 답, 2026-09-15)** PR4 가 남기는 읽기 전용 `대조` 가 (c) 방치를 선택해도 **갈림을 드러낸다** — 어느 칸이 어긋났는지까지 나오므로, (a)·(b) 는 '갈리지 않게 막을 것인가'만 남은 질문이 된다.
