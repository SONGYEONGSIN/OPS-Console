...
### 진행 로그 (2026-07-04, 17차 — 독학학위제/한예종/학위취득 폭넓은 학습 + 하드케이스 규명)

반응형원서를 대학원 외로 확대(독학학위제·한국예술종합학교·학위취득 유형) 배치 학습. number-필드 수정(#808) 후:
- **완주 확인(누적 ~20개 폼, 다양한 대학·전형)**: 외국인(부산대) · 학위취득(동국대전산원·디지털서울문화예술대·숭의여대평생교육원·태재대) · 특수대학원(한양대·대구가톨릭) · 대학원편입(가천대·건국대글로컬·건양대·경상대·대전대) · 일반대학원(공주대·한밭대·부산대한의전) · 교육대학원(인천대·단국대·건국대글로컬) · 한예종(전문사).
- **하드케이스(데이터/폼-specific, 별도 과제)**:
  - **독학학위제(6006008/09) 주민등록번호 재확인**: 원본 주민번호 뒤7자리 `.value`가 `"*******"`(마스킹/암호화) → 복사 불가. **유효 주민번호 생성(생년월일+성별/세기+체크섬) + 암호화 필드 세팅**이 필요.
  - **한예종 예술사(6007042) 고교 졸업일**: DATEFIELD(YYYYMM) 값이 Date 헬퍼에서 클리어됨(예술사 연령 제약 추정). 전문사(6007043)는 완주.
  - **한예종 경연대회(6007040)**: 생년월일 자격게이트 — 현재 테스트 계정으로 불가(8108005 경연대회와 동일).
  - 간헐 진입실패(1141103 등): chromedriver flaky(환경), 코드 아님.
