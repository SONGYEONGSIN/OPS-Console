...
**Teams·SharePoint 파일도 지식망에 넣는다** — 지식망 화면의 '파일로 초안 만들기'에 링크를 붙여넣으면 같은 큐로 간다. 새 파이프라인이 아니라 `read_file` 도구 하나를 붙인 것이다: 서버가 Graph 로 **PDF 변환**(`/content?format=pdf`)해 짧게 사는 주소를 주고, 폴러가 내려받아 Read 한다(우편물 판독과 같은 구조). Word·PPT·Excel·PDF 가 한 경로로 처리된다. 결과는 `제안/` 초안이고 본 위치로는 사람이 옮긴다. 링크 해석은 Graph `/shares/{token}` 에 맡긴다 — 채널·채팅·공유 링크가 모양이 제각각이라 직접 파싱하면 새 형태마다 깨진다.
