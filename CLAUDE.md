...
**담당 대학·서비스 물량 카드는 시트별로 갈라 적는다**(`SheetBreakdownCard`). 모양은 운영리포트 `계약 체결`(`ContractSheetCard`) 그대로 — 카드 머리가 총계를 들고 그 아래 세로 구분선으로 칸을 나눈다(사용자 지시 2026-09-23: 카드 **아래** 따로 표를 뒀더니 같은 숫자를 두 군데서 말했다). 칸이 넷이라 두 카드가 **두 칸씩** 쓰므로 KPI 그리드가 `md:grid-cols-6` 이고, 숫자 하나짜리 카드 둘이 앞에 선다 — 넓은 카드가 먼저 오면 모바일 두 칸 그리드에서 그 옆이 빈 채로 줄이 바뀐다.

**칸의 합은 카드 머리보다 크다** — 한 대학이 원서접수·PIMS·성적산출에 동시에 걸려 있어 467곳 ↔ 295곳이다. 맞추려고 대학을 한 시트에만 넣으면 그 대학의 다른 업무가 사라지므로, 카드 묶음 아래 한 줄이 그 이유를 적는다. 총계는 카드와 **같은 `summary` 객체**를 쓴다 — 거기서 다시 세면 두 숫자가 갈린다. 건수 원천이 그 업무를 아예 안 들면 `원천 없음` 이다(성적산출) — 0 은 '일이 없다' 라서 못 센 것과 갈라 적는다. 상담앱은 칸으로 안 세우지만(사용자 지시 2026-09-22) 그 칸 25개는 담당 대학 총계에 그대로 든다.
