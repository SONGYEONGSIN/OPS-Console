...
- **원서접수**: `closing_services` (Moa 스크래핑. 신규 적재 + **바뀐 값 갱신**, 스크랩에서 빠진 건은 남는다 — 지우지 않는다)
...
## 업무배정 (총괄장 대학배정 · 관리 업무배정)

배정 원장(`assignments`)은 **배정 엑셀이 정본**이고 DB는 그 미러다. 시트 넷이 운영자 배정을 담는다 — `02. 배정리스트` / `03. 대학원` / `04. PIMS` / `06. 성적산출`. 업무종류 → 시트는 `schemas.ts` 의 `SERVICE_KIND_SHEETS` 하나가 답하고, `queries.ts` 의 `SHEET_NAMES` 가 거기서 파생된다.

**화면이 둘이고 권한이 갈린다.** 총괄장 `대학배정`(`/dashboard/assignments`)은 *누가 무엇을 맡고 있는가* 를 전원이 보는 자리이고, 관리 `업무배정`(`/dashboard/work-assignment`, **adminOnly**)은 *누구에게 얼마나 줄 것인가* 를 정하는 자리다. 가드는 `requireMenu(slug)` 하나뿐이다 — 페이지가 `permission === "admin"` 을 따로 보면 `canViewMenu` 와 두 벌이 되어, 한쪽만 바뀌는 날 메뉴는 숨었는데 주소로는 열린다.

**업무 흐름은 2월 종료 → 3월 연간 배정 → 이후 모니터링**이다. 그래서 연간 배정(`assignment-year-rollover`)에 **cron 이 없다** — 수동 버튼이다. 중간에 들어오는 서비스만 건건히 배정한다.

탭 셋: **배정현황**(기본) · **3월배정** · **신규배정**. 이름이 업무 흐름을 그대로 부른다 — 연간 배정은 3월에 하는 일이고, 그 뒤에 들어오는 것이 신규배정이다(#1214).

### 건수 원천이 학년도로 갈린다

`FROZEN_IMPORT_LAST_YEAR = 2026` 이 경계다. **시계가 아니라 데이터의 사실**이라 해가 넘어가도 안 움직인다 — `services` 는 구글 시트 임포트가 2026-02-28 에 멈춘 표이고(2026학년도 2,511건 ↔ `closing_services` 2건), `closing_services` 는 살아 있는 미러다(2027학년도 983건 ↔ `services` 0건). 과거 학년도는 담당자도 `services.operator_email` 에서 온다(99.0% 채움). **과거에는 목표·편차를 내지 않는다** — `operators.tenure_group` 이 오늘 값 하나뿐이라 그때 존재한 적 없는 목표가 나온다.

원천이 바뀌면 **화면이 그걸 말해야 한다.** 2,511 → 983 은 말 안 하면 '물량 60% 감소' 로 읽힌다.

**서비스 물량 카드는 원천 총량이지 사람별 건수의 합이 아니다.** 건수 키가 `대학|업무종류` 뿐이라 하위유형이 갈린 대학(수시는 A, 정시는 B)은 **양쪽 모두에게 전량이 붙는다** — 실측 2026-09-22 로 그런 키가 36개였고 사람별 합 1,307 이 원천 1,112 보다 210건 컸다. 총량으로 두면 `안 붙음` 카드와 분모가 같아져 '1,112 중 15건이 안 붙었다' 로 읽힌다. 사람별 열(`서비스 건수`·밀도·목표 대비)은 그 36개 키에서 여전히 과대평가다 — 정확히 나누려면 건수를 하위유형까지 세야 하는데, 마감 `category` 25종과 원장 `subtype` 5종이 다른 어휘라 별칭 표를 하나 더 만들어야 한다.

**담당 대학·서비스 물량 카드는 시트별로 갈라 적는다**(`SheetBreakdownCard`). 모양은 운영리포트 `계약 체결`(`ContractSheetCard`) 그대로 — 카드 머리가 총계를 들고 그 아래 세로 구분선으로 칸을 나눈다(사용자 지시 2026-09-23: 카드 **아래** 따로 표를 뒀더니 같은 숫자를 두 군데서 말했다). 칸이 넷이라 두 카드가 **두 칸씩** 쓰므로 KPI 그리드가 `md:grid-cols-5` 다(배정 대상 1 + 담당 대학 2 + 서비스 물량 2). 좁은 화면에서는 셋 다 한 줄씩 쓴다 — 한 칸짜리 카드가 혼자 남으면 그 옆이 빈 채로 줄이 바뀐다.

**칸의 합은 카드 머리보다 크다** — 한 대학이 원서접수·PIMS·성적산출에 동시에 걸려 있어 467곳 ↔ 295곳이다. 맞추려고 대학을 한 시트에만 넣으면 그 대학의 다른 업무가 사라지므로, 카드 묶음 아래 한 줄이 그 이유를 적는다. 총계는 카드와 **같은 `summary` 객체**를 쓴다 — 거기서 다시 세면 두 숫자가 갈린다. 건수 원천이 그 업무를 아예 안 들면 `원천 없음` 이다(성적산출) — 0 은 '일이 없다' 라서 못 센 것과 갈라 적는다.

**`안 붙음` 은 카드가 아니라 카드 묶음 아래 문구다**(사용자 지시 2026-09-23) — 숫자 하나뿐인 카드가 옆의 시트별 카드 높이에 맞춰 늘어나 속이 빈 채로 서 있었다. **0 일 때도 적는다**: 0 을 지우면 '안 붙은 것이 없다' 와 '아직 안 세어 봤다' 가 화면에서 같아지고, 231건이 사라진 것을 못 봤던 이유가 바로 그것이다. 0 이 아닐 때만 무엇을 하라고 덧붙인다. 숫자는 **한 텍스트 노드에** 넣는다 — `<span>` 으로 쪼개면 사람이 읽는 문장은 같아 보여도 그 문장을 텍스트로 집을 수 없다. 상담앱은 칸으로 안 세우지만(사용자 지시 2026-09-22) 그 칸 25개는 담당 대학 총계에 그대로 든다.

### 대학명은 원천마다 다르게 불린다 — 읽을 때 원장 쪽으로 맞춘다

실측(2026-09-21)으로 마감 983건 중 **231건(23.5%)** 이 어느 담당자에게도 안 붙고 있었다. 표의 합만 보면 멀쩡해서 아무도 못 봤다. `canonicalUniversity(name, workKind)` 가 세 단계로 맞춘다.

1. **별칭**(`university-aliases.ts`, 19줄) — 사람이 확인한 것만. `한양대학교 대학원` → `한양대학교(서울)`(원장 대학원 칸이 (서울) 하나뿐), `한경대학교` → `한경국립대학교`(2024 교명 변경), `독학학위제` → `평생교육진흥원(독학사)`. **규칙보다 먼저** 본다 — 순서가 반대면 `한양대학교 대학원` 이 원장에 없는 `한양대학교` 가 된다
2. **대학원 접미사 규칙** — `category` 가 이미 대학원이라고 말하므로 이름 끝의 `대학원` 은 중복이다. `성균관대학교 대학원` → `성균관대학교`. 괄호 안 캠퍼스는 남긴다(`동국대학교(서울) 대학원`)
3. **물량이 아닌 대학 제외** — `진학대학교` 는 테스트 대학이다(`엔터 결제 테스트`·`웹 모의해킹 테스트`)

**원장은 안 건드린다.** 엑셀이 정본이고 마감·발표가 미러라, 미러 쪽 표기만 옮긴다. 정규화는 건수·구간·과거 담당 칸 **셋이 같은 함수**(`cellOf`)를 쓴다 — 한 곳만 고치면 키가 어긋나는데, 그건 에러가 아니라 '그 사람은 아무것도 안 맡았다' 로 화면에 나온다.

그리고 **남은 것을 화면이 말한다**(`unmatchedVolume` → 배정현황 배너, 0이면 안 띄운다). 규칙보다 이 배너가 오래 간다 — 이게 없어서 23.5%가 사라진 것을 못 봤다. 별칭은 DB 표가 아니라 **커밋된 상수**다: 연 몇 회 바뀌는 값이라 마이그레이션·RLS·관리 화면을 세울 만큼 움직이지 않고, 새 갈림은 배너가 알려 준다. 30줄을 넘거나 배포 없이 고쳐야 하면 그때 표로 올린다.

### 신규배정 — 줄이 되는 규칙

줄의 출처가 셋이고 규칙이 다르다: ①원장 미배정 칸 ②원장 '연결 안 됨'(**요청 버튼 없음** — 고칠 것은 주소다) ③마감에 있고 원장에 없으며 **아직 시작 전**.

③의 시작 전 조건이 핵심이다. 조건 없이 세면 26개가 뜨는데 그중 23개가 표기 갈림이라 **늘 켜진 경고등**이 된다. 그래도 **거르지 않고** 비슷한 이름을 나란히 적는다(공통접두사 4자 — 실측 분포에 2·3자가 없어 빈 구간이 경계를 그어 줬다). 걸러 버리면 `서울대학교 시흥캠퍼스` 같은 진짜 새것이 조용히 사라진다.

한 줄이 네 물음에 답한다 — 어느 시트에 / 언제 시작 / 어느 그룹에 / **그때** 여유가 있나. 마지막은 `workloadWindows` 에 **서비스 시작일**을 넘겨 다시 잰다. '지금 여유 있나' 를 보면 12월 서비스를 9월 부하로 판단한다.

### 판정은 폴러가, 승인은 사람이

`[배정 요청]` 은 원장을 바꾸지 않는다 — `assignment_propose_requests` 에 적재하고 회사 PC 폴러(`scripts/assignments/propose-local.mjs`, Agent SDK)가 가져가 판정한다. 결과는 3월배정 탭에 배치로 쌓이고, **적용 전까지 원장은 그대로이며 반려가 기본 선택지**다. server action 에는 라우트 가드가 없어 `requireAdmin` 을 거기서 다시 본다.

원장을 쓰는 server action 은 **두 주소를 짝으로** 다시 그린다(`paths.ts` + `__tests__/paths.test.ts`) — 라우트가 갈린 뒤 한쪽만 그리면 승인한 배치가 계속 pending 으로 보이고, 에러는 안 난다.

**백업자는 배정이 아니다.** 시트 블록의 여섯 번째 칸이 백업자인데 파서가 이를 하위유형으로 읽어 원장에 넣고 있었다(`parse.ts` 의 `isBackupLabel`). 2027학년도는 그 칸이 비어 있어 드러나지 않았고, 2026학년도엔 22행이 차 있었다 — 채워졌다면 `원서접수|백업자|운영` 이 부하를 부풀리고 제안의 이동 대상이 됐을 것이다.

**반려는 그 학년도를 닫지 않는다.** 새 연간 제안을 막는 것은 **검토 대기(pending)** 배치뿐이다 — DB 의 부분 유니크가 pending 에만 걸려 있는데 조회가 전 상태를 막고 있어서, 한 번 반려하면 그 학년도에 다시는 제안을 만들 수 없었다(#1212, `hasPendingAnnualBatch`).

**확정된 원장은 `(앱) 배정확정` 시트로 내보낸다**(총괄장 대학배정의 `ExportAssignments`, #1213). PIMS 는 하위유형 두 칸(FULL·환충)으로 나가고, **백업자는 안 나간다** — 배정이 아니기 때문이다.

`assignment-unassigned-sweep`(평일 09:30)은 **감지만 한다** — 요청을 만들지 않는다. 누구에게 줄지는 신규배정에서 사람이 정한다.

설계 문서: `docs/superpowers/specs/2026-09-21-work-assignment-menu-design.md`
...
**Teams·SharePoint 파일도 지식망에 넣는다** — 지식망 화면 '초안 만들기' 탭에 링크를 붙여넣으면 같은 큐로 간다. 새 파이프라인이 아니라 `read_file` 도구 하나를 붙인 것이다: 서버가 Graph 로 **PDF 변환**(`/content?format=pdf`)해 짧게 사는 주소를 주고, 폴러가 내려받아 Read 한다(우편물 판독과 같은 구조). Word·PPT·Excel 이 한 형식으로 모인다. **원본이 이미 PDF 면 변환을 건너뛴다** — 변환 서비스는 PDF 를 입력으로 안 받는데 거절이 302 뒤에 온다(주소는 멀쩡히 주고 열면 406 `InputFormatNotSupported`), 그래서 서버는 성공으로 보이고 폴러에서 '내려받기 406' 으로 죽었다. 재료는 셋이다 — **링크**(이미 올라간 것) · **파일 올리기**(내 PC 의 것) · **직접 입력**(메일 본문처럼 파일이 아예 없는 것). 올린 파일은 **볼트 밑 `첨부/`** 에 두고 그 주소를 링크처럼 넘긴다 — 읽는 경로를 두 벌로 만들지 않는다(4MB 를 넘으므로 업로드 세션으로 올린다). 폴더 env 를 새로 두지 않은 건 **사람이 안 넣은 채로 기능이 죽기 때문**이고, 볼트 위치는 인덱서가 이미 안다. `첨부` 는 인덱서 SKIP_DIRS 에 있다. 직접 입력은 `read_file` 을 안 거친다.

**답은 그 자리로 돌아온다** — 진행 단계가 흐르고, 끝나면 답이 뜨고, 되물으면 거기서 답해 이어간다. 탭이 URL 이라 다른 탭에 다녀오면 화면이 죽으므로, 돌아오면 도는 중이던 요청을 이어받는다(`running-draft.ts`, `page_context` 가 표식이라 세 칸이 같은 값을 써야 한다). 편도로 뒀더니 에이전트가 33쪽 취업규칙에 '연도별 값을 넣을까요, 어느 칸에 넣을까요'를 되물었고 그 말이 아무도 안 보는 행에 갇혀 초안 없이 끝났다(2026-08-25). 되묻기는 좋은 질문이라 막을 게 아니라 답할 자리를 준다. 폴링은 어시스턴트 창과 같은 `features/assistant/poll-request.ts` 를 쓴다. 결과는 `제안/` 초안이고 본 위치로는 사람이 옮긴다 — **'검토 대기' 탭에서 문서를 열면 `지식망 옮기기`**(서버가 Graph 로 이동, 폴러 불필요) 또는 채팅의 `promote_doc`. 분류는 초안이 선언한 것을 그대로 쓴다 — 화면에서 고르게 하면 frontmatter 와 폴더가 어긋나 `미분류` 로 떨어진다. 링크 해석은 Graph `/shares/{token}` 에 맡긴다 — 채널·채팅·공유 링크가 모양이 제각각이라 직접 파싱하면 새 형태마다 깨진다.
