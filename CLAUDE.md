...
그리고 **남은 것을 화면이 말한다**(`unmatchedVolume` → 배분현황 배너, 0이면 안 띄운다). 규칙보다 이 배너가 오래 간다 — 이게 없어서 23.5%가 사라진 것을 못 봤다. 별칭은 DB 표가 아니라 **커밋된 상수**다: 연 몇 회 바뀌는 값이라 마이그레이션·RLS·관리 화면을 세울 만큼 움직이지 않고, 새 갈림은 배너가 알려 준다. 30줄을 넘거나 배포 없이 고쳐야 하면 그때 표로 올린다.
...
`[배정 요청]` 은 원장을 바꾸지 않는다 — `assignment_propose_requests` 에 적재하고 회사 PC 폴러(`scripts/assignments/propose-local.mjs`, Agent SDK)가 가져가 판정한다. 결과는 제안 탭에 배치로 쌓이고, **적용 전까지 원장은 그대로이며 반려가 기본 선택지**다. server action 에는 라우트 가드가 없어 `requireAdmin` 을 거기서 다시 본다.
