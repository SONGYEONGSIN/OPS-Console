...
**시트별 현황**(카드 아래 표)은 그 총량이 어느 시트의 것인지 나눈다. **행의 합은 합계와 안 맞는다** — 한 대학이 원서접수·PIMS·성적산출에 동시에 걸려 있어 행의 합 467곳 ↔ 중복 제거 295곳이다. 맞추려고 대학을 한 시트에만 넣으면 그 대학의 다른 업무가 사라지므로, 합계 줄이 '중복 제거' 라고 적는다. 합계는 카드와 **같은 `summary` 객체**를 쓴다 — 여기서 다시 세면 두 숫자가 갈린다. 상담앱은 행으로 안 세우지만(사용자 지시 2026-09-22) 그 칸 25개는 담당 대학 합계에 그대로 든다.
