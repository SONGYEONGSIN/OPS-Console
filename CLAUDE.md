...
- **로그인 점유(리스)** `sms_code_lease` 한 줄(TTL 180초). 스크래퍼는 제출 전에 `reset`(비우기+점유), 제출 후 `pop`(꺼내며 삭제·반납). 다른 소비자가 점유 중이면 409 — **폴백하지 않고 중단한다.** 폴백하면 SMS 두 통이 한 폰에 겹쳐 서로의 코드를 가져가고, 틀린 코드 제출은 캡차 잠금으로 자동화 전체를 세운다. 비점유자의 `pop` 도 409(0행이면 '아직 안 왔다'와 구분이 안 된다).
