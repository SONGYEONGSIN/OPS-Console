...
def build_prompt(services: list[dict]) -> str:
    ...
    return (
        ...
        "4. 문구가 'X시부터 N시간(분) 단위로 업데이트' 형태이고 X가 접수 시작 시각이면, "
        "스케줄의 첫 실행이 X가 아니라 X+N이어도 정상이다. 접수 시작을 기준점으로 주기를 "
        "안내한 것이며, 접수 시작 시점에는 경쟁률이 0이라 갱신이 없을 수 있다. 이 경우 "
        "이상으로 보고하지 마라.\n"
        "5. 확신이 없으면 보고하지 마라. 추측 금지.\n\n"
