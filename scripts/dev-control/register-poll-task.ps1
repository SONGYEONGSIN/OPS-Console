...
#
# 5분마다 poll-local.ps1을 실행해 웹 '지금 분석' 요청(pending)을 확인.
# pending이 있으면 dev-control-analyze.mjs <serviceId>를 실행하고, 없으면 즉시 종료(가벼움).
# dev-control-analyze.cmd(매일 08:30 전체 실행)와 별개로, 웹 버튼 기반 on-demand 실행용.
#
# 기본(로그온 시에만):
#   powershell -NoProfile -ExecutionPolicy Bypass -File scripts/dev-control/register-poll-task.ps1
# 무인(로그오프 상태에서도 — S4U, 비밀번호 불필요): 권장
#   powershell -NoProfile -ExecutionPolicy Bypass -File scripts/dev-control/register-poll-task.ps1 -Unattended
# 무인(비밀번호 저장 폴백):
#   powershell -NoProfile -ExecutionPolicy Bypass -File scripts/dev-control/register-poll-task.ps1 -StorePassword
#
...
$desc = "개발탭 수동 분석 요청 폴러. 5분마다 pending 확인 → dev-control-analyze.mjs <serviceId> 실행."
