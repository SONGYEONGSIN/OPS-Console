...
export function buildStoryPrompt(payload, issueNo) {
  ...
  return `당신은 사내 운영부 주간 뉴스레터의 편집자입니다. 아래 주간 데이터를 바탕으로 스티비 뉴스레터 스타일의 밝고 친근한 한국어 문구를 작성하세요.
...
- 존댓말, 사내 운영부 동료 대상. 입사 1년 차부터 10년 차까지 모두 읽는 글이므로 쉬운 표현으로.
- 근속 기념일이 있으면 intro 또는 schedule 이야기에 자연스럽게 축하를 담으세요.
