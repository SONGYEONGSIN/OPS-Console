// 입금 매칭 자동화 — GitHub Actions cron 진입점.
// OPS-Console API route `/api/automations/run`를 CRON_SECRET 인증으로 호출.
// 사용 (local):
//   OPS_CONSOLE_BASE_URL=http://localhost:3000 CRON_SECRET=... node scripts/receivables-deposit-match.mjs
...
const url = `${BASE_URL.replace(/\/$/, "")}/api/automations/run?jobId=receivables-deposit-match`;
